...
- Plateau des Épaves : épaves procédurales bornées (pas de `.nbt`, question laissée en suspens depuis
  ...
  (`TB-WORLD-005`) ;
- récifs rocheux autour des avant-postes de phare (`PortArchetype.LIGHTHOUSE_OUTPOST`), déterministes
...
- retours du premier vrai test en jeu (`TB-CORE-016`) : cadre du nom de biome retiré (texte simple) ;
  chaque port a désormais au moins deux PNJ au lieu d'un seul dans la grande majorité des cas ; chance
