...
| Confort | Counter Day | Compteur de jours en survie, écho au repère de pacing « 100 jours » |
...
- JourneyMap (choix initial) a été remplacé par Xaero's Minimap après deux crashes identiques
  (`TB-CORE-013`), puis **remis** à la demande explicite de l'utilisateur (`TB-CORE-017`) : troisième
  tentative avec `6.0.0-beta.52`, choisie pour son volume de téléchargements (4,7 M, un vrai signal de
  stabilité en usage réel) plutôt que « la plus récente » comme les deux essais précédents qui ont
  échoué. Risque de crash assumé explicitement par l'utilisateur.
