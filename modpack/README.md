# Tidebound — Into the Deep (Devpack 0.40.0-alpha)
...
- Xaero's Minimap, Waystones et Balm ;
...
- Inventory Profiles Next (+ libIPN et Kotlin for Forge, requis comme fournisseur de langage),
  équivalent moderne d'Inventory Tweaks (tri automatique) ;
- Counter Day, pour suivre le nombre de jours en survie.

Le JAR Tidebound n'est pas téléchargé par CurseForge. Après import du dossier seul, copier
`tidebound-0.40.0-alpha.jar` dans le dossier `mods` de l'instance.

Le workflow GitHub Actions automatise cette étape : son artifact contient
`Tidebound_Devpack_0.40.0-alpha.zip`, directement importable dans CurseForge, ainsi que le JAR seul.
